    ...
    next.addEventListener("click", () => {
      for (let i = 0; i < pageNum.length; i++) {
        if (pageNum[i].innerHTML === sessionStorage.getItem("page") && pageNum[i].innerHTML !== "10") {
          ...
          const url = `https://api.themoviedb.org/3/discover/movie?api_key=775c82528794f6f38cd6a9bd7c080365&page=${sessionStorage.getItem("page")}&with_genres=${sessionStorage.getItem("genreId")}&language=en-US`;
          ClearMain();
    ...
    previous.addEventListener("click", () => {
      for (let i = 0; i < pageNum.length; i++) {
        if (pageNum[i].innerHTML === sessionStorage.getItem("page") && i !== 0) {
          sessionStorage.setItem("page", `${pageNum[i - 1].innerHTML}`);
          const url = `https://api.themoviedb.org/3/discover/movie?api_key=775c82528794f6f38cd6a9bd7c080365&page=${sessionStorage.getItem(
            "page"
          )}&with_genres=${sessionStorage.getItem("genreId")}&language=en-US`;
          ClearMain();
          pageNum[i - 1].classList.add("selected-page");
          pageNum[i].classList.remove("selected-page");
          FetchMoviesUrl(url);
